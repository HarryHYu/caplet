...
export function actionDetails({ resume, studyTask, recommendation, fallbackHref, fallbackTitle, fallbackDetail, fallbackMode = 'diagnostic', source }) {
  ...
  return { type: fallbackMode, href: fallbackHref || `/practice?subject=economics&mode=diagnostic&source=${source}`, eyebrow: 'Best place to start', title: fallbackTitle || 'Take the quick Economics diagnostic', detail: fallbackDetail || 'Five questions give Caplet the evidence it needs to choose your next useful activity.', mode: fallbackMode };
}
