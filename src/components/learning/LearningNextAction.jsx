...
export default function LearningNextAction({ resume, studyTask, recommendation, fallbackHref, fallbackTitle, source = 'learning', className = '', trackingEnabled = true, variant = 'default' }) {
  ...
  return (
    <Link
      to={action.href}
      onClick={handleClick}
      className={`group flex flex-col gap-6 rounded-3xl bg-[color:var(--mark-blue)] p-7 text-accent-contrast shadow-glow card-lift focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-4 md:flex-row md:items-center md:justify-between md:p-9 ${className}`}
    >
      <div className="flex items-start gap-4">
        <span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-accent-contrast/10"><SparklesIcon className="h-6 w-6" aria-hidden="true" /></span>
        <div><p className="text-xs font-bold uppercase tracking-[0.14em] text-accent-contrast/65">{action.eyebrow}</p><h2 className="mt-2 font-display text-2xl font-extrabold tracking-tight text-accent-contrast md:text-3xl">{action.title}</h2><p className="mt-2 max-w-2xl text-sm font-medium leading-relaxed text-accent-contrast/80">{action.detail}</p></div>
      </div>
      <span className="inline-flex shrink-0 items-center justify-center gap-2 rounded-2xl bg-accent-contrast px-5 py-3 text-sm font-bold text-accent">Start now <ArrowRightIcon className="h-4 w-4 transition-transform group-hover:translate-x-1" aria-hidden="true" /></span>
    </Link>
  );
}
