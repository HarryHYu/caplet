...
export default function TabletDashboardNavbar() {
    ...
    return (
        ...
        >
            <div className="mx-auto flex h-16 max-w-[1400px] items-center gap-3 px-6 md:px-8">
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <Link
                                ...
                                className={`inline-flex min-h-11 items-center rounded-xl px-3 text-sm font-bold tracking-[0.03em] transition-colors ${
                ...
                <div className="ml-auto flex shrink-0 items-center gap-2">
                    ...
                    <div className="relative" ref={menuRef}>
                        <button
                            ...
                            className={`flex h-11 w-11 items-center justify-center rounded-full border p-1.5 transition-colors ${
                        ...
                        {showUserMenu && (
                            <div id="tablet-account-navigation" className="absolute right-0 top-full mt-2 w-48 overflow-hidden rounded-xl border border-line-soft bg-surface-raised py-1.5 shadow-[0_8px_32px_rgba(0,0,0,0.12)]">
                                <div className="mb-1 border-b border-line-soft px-3 py-2">
                                    <p className="truncate text-xs font-medium text-text-dim">{user?.email || ''}</p>
                                </div>
                                <Link to="/settings" onClick={() => setShowUserMenu(false)} className="block px-3 py-2 text-sm text-text-primary transition-colors hover:bg-surface-soft">Settings</Link>
                                <div className="my-1 border-t border-line-soft" />
                                <button type="button" onClick={logout} className="w-full px-3 py-2 text-left text-sm text-red-500 transition-colors hover:bg-red-50 dark:hover:bg-red-900/20">Sign out</button>
                            </div>
                        )}
                    </div>
                </div>
