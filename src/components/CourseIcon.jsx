import { getCourseIcon } from '../lib/courseIcons';

const sizes = {
  sm: 'h-9 w-9 rounded-xl',
  md: 'h-11 w-11 rounded-2xl',
  lg: 'h-14 w-14 rounded-2xl',
  hero: 'h-full w-full rounded-none',
};

/** A local fallback mark for courses without a supplied thumbnail. */
export default function CourseIcon({ course, size = 'md', className = '' }) {
  const { key, Icon, tone } = getCourseIcon(course);

  return (
    <span
      aria-hidden="true"
      data-testid="course-icon"
      data-course-icon={key}
      className={`grid shrink-0 place-items-center border border-line-soft ${sizes[size] || sizes.md} ${tone} ${className}`}
    >
      <Icon className="h-1/2 w-1/2" strokeWidth={1.8} />
    </span>
  );
}
