...
export function normalizeQuestion(question, fallbackSubject = 'economics') {
  ...
  return {
    ...
    sourceTitle: source.title || source.name || source.provenance || '',
    sourceUrl: source.url || source.sourceUrl || source.documentUrl || '',
    sourceAuthor: source.author || source.provider || '',
    sourceYear: source.year ? String(source.year) : '',
    sourceNotes: source.notes || source.description || source.provenance || '',
