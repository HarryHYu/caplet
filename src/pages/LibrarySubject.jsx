  ...
  return (
    <div className="min-h-screen bg-surface-body pb-28 pt-24 selection:bg-accent selection:text-accent-contrast md:pt-28">
      <div className="container-custom">
        ...
        <header className="animate-rise mt-8 grid gap-8 rounded-3xl block-blue p-7 shadow-card md:grid-cols-[1fr_auto] md:items-end md:p-10">
          ...
          <div className="flex flex-wrap gap-3">
            <Link to={pack.studentLinks.diagnostic} className="btn-primary focus-ring">Start diagnostic <ArrowRightIcon className="h-4 w-4" aria-hidden="true" /></Link>
            <Link to={pack.studentLinks.mastery} className="btn-secondary focus-ring">View mastery</Link>
          </div>
        </header>

        <section className="reveal-stagger mt-8 grid gap-4 md:grid-cols-3">
          <div className="rounded-3xl bg-surface-raised p-6 shadow-card"><CheckBadgeIcon className="h-7 w-7 text-accent" /><p className="mt-4 text-xs font-bold uppercase tracking-[0.12em] text-text-dim">Published syllabus</p><p className="mt-2 font-display text-xl font-extrabold">{pack.syllabusName || pack.title}</p></div>
          <div className="rounded-3xl bg-surface-raised p-6 shadow-card"><BookOpenIcon className="h-7 w-7 text-accent" /><p className="mt-4 text-xs font-bold uppercase tracking-[0.12em] text-text-dim">Mapped outcomes</p><p className="mt-2 font-display text-3xl font-extrabold">{outcomes.length}</p></div>
          <Link to={pack.studentLinks.practice} className="card-lift focus-ring group rounded-3xl block-green p-6 shadow-card transition-shadow hover:shadow-card-hover"><ClipboardDocumentCheckIcon className="h-7 w-7 text-accent" /><p className="mt-4 text-xs font-bold uppercase tracking-[0.12em] text-text-dim">Daily practice</p><p className="mt-2 font-display text-xl font-extrabold">Get questions chosen from your evidence</p><span className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-accent">Practise now <ArrowRightIcon className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span></Link>
        </section>
        ...
        <section className="reveal mt-14">
          ...
          <div className="reveal-stagger mt-7 grid gap-6 lg:grid-cols-2">
            {groups.map((group) => (
              <div key={group.title} className="rounded-3xl bg-surface-raised p-6 shadow-card md:p-8">
