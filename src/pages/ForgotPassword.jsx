...
export default function ForgotPassword() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setMessage('If an account can use that email, a password reset link will be sent shortly.');
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <main className="min-h-[100dvh] bg-surface-body px-5 py-20">
      <div className="mx-auto max-w-md rounded-3xl border border-line-soft bg-surface-raised p-7 shadow-[0_24px_50px_-34px_rgba(20,20,18,0.3)] sm:p-9">
        ...
        {message ? (
          <div role="status" className="mt-6 rounded-2xl block-green p-4 text-sm font-medium text-text-primary">{message}</div>
        ) : (
          <form onSubmit={submit} className="mt-7">
            ...
            <button type="submit" disabled={sending} className="btn-primary mt-5 w-full justify-center py-3 disabled:opacity-50">{sending ? 'Sending…' : 'Send reset link'}</button>
          </form>
        )}
