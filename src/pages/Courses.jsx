...
const Courses = () => {
  ...
  useEffect(() => {
    fetchCourses({ search: filters.search });
  ...
  const courseStage = (course) => {
    if (course.metadata?.stage) return String(course.metadata.stage);
    const year = Number(course.metadata?.yearLevel);
    if (year >= 11) return 'Stage 6';
    if (year >= 9) return 'Stage 5';
    if (year >= 7) return 'Stage 4';
    return 'General';
  };
  const visibleCourses = filters.stage ? courses.filter((course) => courseStage(course) === filters.stage) : courses;
  ...
  const showNoMatches = visibleCourses.length === 0 && !loading && !error && hasActiveFilters;
      ...
      <div className="container-custom">
        ...
        <div className="reveal mb-12 flex flex-col gap-5 rounded-3xl bg-surface-soft p-5 sm:flex-row sm:p-6">
          <div className="sm:w-48">
            <label htmlFor="learning-path-stage" className="text-sm font-semibold text-text-dim mb-3 block">Curriculum stage</label>
            <select
              id="learning-path-stage"
              value={filters.stage}
            ...
            >
              ...
              <option value="Stage 4">Stage 4 · Years 7–8</option>
              <option value="Stage 5">Stage 5 · Years 9–10</option>
              <option value="Stage 6">Stage 6 · Years 11–12</option>
