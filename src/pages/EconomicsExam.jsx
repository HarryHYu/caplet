  ...
  return (
    <main className="min-h-screen bg-surface-body py-24 text-text-primary">
      <div className="container-custom max-w-4xl">
        ...
        <header className="mt-10 rounded-3xl block-blue p-7 md:p-10">
        ...
        <section className="mt-8 grid gap-5 md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-line-soft bg-surface-raised p-6"><p className="text-xs font-extrabold uppercase tracking-wide text-text-dim">Keep the loop going</p><p className="mt-4 text-sm leading-relaxed text-text-muted">Your study plan can now use the latest marked feedback to prioritise what comes next.</p><Link to="/study-plan" className="press focus-ring mt-5 inline-flex items-center gap-2 rounded-xl bg-accent px-4 py-3 text-sm font-extrabold text-accent-contrast">Open my study plan <ArrowRightIcon className="h-4 w-4" /></Link></div>
        </section>
...
export default function EconomicsExam() {
  ...
  if (!pack) return <main className="min-h-screen bg-surface-body grid place-items-center p-8"><div className="max-w-md text-center"><h1 className="font-display text-2xl font-extrabold text-text-primary">Practice pack not found</h1><p className="mt-2 text-text-muted">The link may be old or the pack may no longer be available.</p><Link to="/library/economics/exam-practice" className="btn-primary mt-5">Choose a practice pack</Link></div></main>;
  ...
  if (!session) return <main className="min-h-screen bg-surface-body py-24 text-text-primary"><div className="container-custom max-w-3xl"><Link to="/library/economics/exam-practice" className="inline-flex items-center gap-2 text-sm font-bold text-text-muted hover:text-accent"><ArrowLeftIcon className="h-4 w-4" /> Economics practice</Link><section className="mt-10 rounded-3xl border border-line-soft bg-surface-raised p-7 md:p-10"><p className="text-xs font-extrabold uppercase tracking-wide text-accent">Timed written practice</p><h1 className="mt-2 font-display text-4xl font-extrabold tracking-tight">{pack.title}</h1>{questions.length ? <><p className="mt-5 max-w-2xl text-base leading-relaxed text-text-muted">Work through {questions.length} written responses in one focused sitting. Your draft autosaves, and you’ll get feedback after you submit.</p><div className="mt-8 grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-surface-soft p-4"><ClockIcon className="h-5 w-5 text-accent" /><p className="mt-3 text-sm font-extrabold">{Math.min(90, Math.max(30, questions.reduce((sum, q) => sum + q.markValue * 2, 0)))} minutes</p></div><div className="rounded-xl bg-surface-soft p-4"><CheckCircleIcon className="h-5 w-5 text-accent" /><p className="mt-3 text-sm font-extrabold">{questions.length} responses</p></div><div className="rounded-xl bg-surface-soft p-4"><SparklesIcon className="h-5 w-5 text-accent" /><p className="mt-3 text-sm font-extrabold">Feedback after you submit</p><p className="mt-1 text-xs font-medium text-text-dim">Caplet uses AI to review written responses.</p></div></div>{error ? <p role="alert" className="mt-5 text-sm font-bold text-text-error">{error}</p> : null}<button type="button" onClick={start} disabled={starting} className="press focus-ring mt-8 rounded-xl bg-accent px-6 py-3 text-sm font-extrabold text-accent-contrast disabled:opacity-50">{starting ? 'Starting…' : 'Start practice'}</button></> : <div className="mt-6 rounded-2xl bg-surface-soft p-5"><h2 className="font-display text-xl font-extrabold">Written practice is coming later</h2><p className="mt-2 text-sm leading-relaxed text-text-muted">You can browse this pack now, but its written questions are still being reviewed. There is no broken start button to take you into an empty session.</p><Link to={`/library/economics/exam-practice/${pack.id}`} className="btn-secondary mt-5">Browse pack</Link></div>}</section></div></main>;
