...
// Extract financial data from user message using AI
const extractFinancialData = async ({ message, currentState }) => {
  try {
    if (!process.env.OPENAI_API_KEY) {
      console.error('OPENAI_API_KEY is not set');
      return null;
    ...
    for (const model of models) {
      try {
        console.log(`Extracting financial data with model: ${model}`);

        const prompt = `You are a financial data extraction system. Extract structured financial information from the user's message.

USER MESSAGE:
"${message}"

CURRENT FINANCIAL STATE:
- Monthly Income: $${(currentState.monthlyIncome || 0).toLocaleString()}
- Monthly Expenses: $${(currentState.monthlyExpenses || 0).toLocaleString()}
- Accounts: ${JSON.stringify(currentState.accounts || [])}
- Debts: ${JSON.stringify(currentState.debts || [])}
- Goals: ${JSON.stringify(currentState.goals || [])}

INSTRUCTIONS:
Extract any financial information mentioned in the message:
1. Income: Look for salary, wages, income amounts, pay raises, new jobs
2. Expenses: Extract specific expenses mentioned (rent, food, bills, subscriptions, purchases, etc.)
3. Accounts: Bank accounts, savings accounts mentioned
4. Debts: Loans, credit cards, debts mentioned
5. Goals: Financial goals, savings targets, purchase plans mentioned
6. Major Events: Job changes, large purchases, windfalls, emergencies

Only extract information that is EXPLICITLY mentioned in the message. Don't make assumptions.
If no information is mentioned for a category, return null or empty.

OUTPUT FORMAT (MUST BE VALID JSON):
{
  "monthlyIncome": <number or null if mentioned>,
  "expenses": {
    "rent": <number or null>,
    "food": <number or null>,
  ...
  "goals": [{"name": "<goal name>", "targetAmount": <number>, "targetDate": "<date or null>", "currentAmount": <number or null>}],
  "majorEvents": [{"type": "<job_change/purchase/windfall/emergency>", "description": "<description>", "amount": <number or null>}]
}
...
Extract the financial data now:`;
        ...
        const response = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a financial data extraction system. Extract structured financial information from user messages. Always respond with valid JSON.'
            },
            {
              role: 'user',
              content: prompt
        ...
        console.log(`Financial data extracted successfully with ${model}`);
        return extractedData;
      } catch (modelError) {
        console.error(`Error extracting data with model ${model}:`, modelError.message);
        lastError = modelError;
        if (modelError.message?.includes('model') || modelError.message?.includes('not found')) {
          continue;
        }
    ...
    throw lastError || new Error('All models failed for data extraction');
  } catch (error) {
