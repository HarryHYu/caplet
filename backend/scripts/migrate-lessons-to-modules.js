...
async function migrate() {
  ...
  // 2) Check if lessons has courseId column (old schema)
  let hasCourseId = false;
  if (dialect === 'sqlite') {
    const info = await sequelize.query(
      "SELECT name FROM pragma_table_info('lessons') WHERE name='courseId'",
      { type: sequelize.QueryTypes.SELECT }
    );
    hasCourseId = info && info.length > 0;
  } else {
    const [rows] = await sequelize.query(`
      SELECT column_name FROM information_schema.columns
      WHERE table_name = 'lessons' AND column_name = 'courseId'
    `);
    hasCourseId = rows && rows.length > 0;
  }
  ...
  if (dialect === 'sqlite') {
    const info = await sequelize.query(
      "SELECT name FROM pragma_table_info('lessons') WHERE name='moduleId'",
      { type: sequelize.QueryTypes.SELECT }
    );
    hasModuleId = info && info.length > 0;
    if (!hasModuleId) {
      await sequelize.query('ALTER TABLE lessons ADD COLUMN moduleId TEXT REFERENCES modules(id);');
  ...
  const lessonsRaw = dialect === 'sqlite'
    ? await sequelize.query('SELECT id, courseId FROM lessons WHERE courseId IS NOT NULL', { type: sequelize.QueryTypes.SELECT })
    : (await sequelize.query('SELECT id, "courseId" FROM lessons WHERE "courseId" IS NOT NULL', { type: sequelize.QueryTypes.SELECT }));
  ...
  for (const courseId of Object.keys(byCourse)) {
    ...
    const placeholders = ids.map(() => (dialect === 'postgres' ? '?' : '?')).join(',');
    const updateSql = dialect === 'postgres'
      ? `UPDATE lessons SET "moduleId" = ? WHERE id IN (${placeholders})`
      : `UPDATE lessons SET moduleId = ? WHERE id IN (${placeholders})`;
    await sequelize.query(updateSql, { replacements: [mod.id, ...ids] });
